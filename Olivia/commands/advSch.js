module.exports = {
    name: "advSch",
    description: "testing the date command",
    execute(message, commandTerms, client, schedulingArrays) {
        ...
        for(var i = 0; i < 5; i++) {
            if (commandTerms[i] == null) {
                ...
                message.channel.send("`+advSch [text channel] [days]d [hours]h [minutes]m [message]`");
                return;
            }
        }
        ...
        var cancelProcedure = setInterval(() => {
            if(message.channel.lastMessage.content.toLowerCase() == "cancel") {
                message.channel.bulkDelete(4);
                message.author.send("The scheduled message has been cancelled");

                setTimeout(() => { clearInterval(outputInterval) }, 1000);
                clearInterval(cancelProcedure);
                clearInterval(minuteClear);

            } else if (message.channel.lastMessage.content.toLowerCase() == "confirm") {
                message.channel.bulkDelete(4);
            ...
            if(message.channel.lastMessage.content.toLowerCase() == "cancel" || message.channel.lastMessage.content.toLowerCase() == "confirm"){
                 setTimeout(() => {
        ...
        var minuteClear = setTimeout(() => {
            clearInterval(cancelProcedure);
            message.channel.send("No reply. Cancelling procedure.");
            message.author.send("No reply. Cancelling procedure.");
        ...
        var outputInterval = setInterval(() => {
            if (confirmationStatus) {
                ...
                schedulingArrays.push([message.author.id, "<#" + commandTerms[1] + ">", Date.now() + msTime, output]);
                console.log(schedulingArrays);
